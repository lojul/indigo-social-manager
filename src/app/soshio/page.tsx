...
export default function SoshioLanding() {
  return (
    ...
    <nav className="fixed top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <div className="flex items-center gap-8">
          <Link href="/soshio" className="font-heading text-xl font-semibold tracking-tight text-deep-purple">
            Soshio
          </Link>
    ...
    <section id="features" className="bg-muted py-24">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="relative overflow-hidden rounded-[20px] bg-deep-purple p-8 ring-1 ring-black/5 md:col-span-2">
            <div className="flex h-full flex-col justify-between gap-8 lg:flex-row lg:items-center">
              <div className="max-w-[35ch]">
                ...
                <h3 className="mb-3 font-heading text-xl font-medium text-white">AI-powered content</h3>
          ...
          <FeatureCard icon={<CalendarClock className="size-4" />} title="Buffer scheduling" body="Seamlessly push your approved posts directly to your Buffer queue for automated publishing." accent />
          <FeatureCard icon={<TrendingUp className="size-4" />} title="Trend-aware topics" body="Stay relevant with daily suggestions based on what's currently viral in your specific industry niche." tone="muted" />
          <FeatureCard icon={<Languages className="size-4" />} title="Multilingual EN/中文" body="Localize your global presence with perfect translations in English and Chinese." />
          <FeatureCard icon={<Film className="size-4" />} title="AI video reels" body="Transform text prompts into short-form video content complete with transitions and AI-selected music." />
        </div>
      </div>
    </section>
  );
...
function FeatureCard({ icon, title, body, accent, tone = 'white' }: { icon: React.ReactNode; title: string; body: string; accent?: boolean; tone?: 'white' | 'muted' }) {
  ...
  return (
    <div className={`group relative overflow-hidden rounded-[20px] ${bg} p-8 ring-1 ring-black/5 transition-transform hover:-translate-y-1`}>
      <div className={`mb-6 flex size-10 items-center justify-center rounded-xl ${accent ? 'bg-coral-mid text-white' : 'bg-coral-light/10 text-coral-mid'}`}>
        {icon}
      </div>
      <h3 className={`mb-3 font-heading text-lg font-medium ${accent ? 'text-coral-dark' : ''}`}>{title}</h3>
        ...
        <div className="grid items-start gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <p className="mt-6 max-w-[42ch] text-pretty text-muted-foreground">
              Start for free and scale as your audience grows. No hidden fees — just pure creativity.
            </p>
          </div>
            ...
            <div className="flex-1 rounded-[24px] bg-card p-8 ring-1 ring-black/5">
              ...
              <ul className="mt-8 space-y-4 text-sm text-muted-foreground">
                {['1 workspace', '5 posts per month', 'AI post copy', 'Trend search'].map((f) => (
                  <li key={f} className="flex items-center gap-2"><Check className="size-4 text-muted-foreground/50" />{f}</li>
                ))}
              </ul>
              <Link href="/login" className="mt-10 block w-full rounded-full bg-muted py-2.5 text-center text-sm font-medium ring-1 ring-black/5 transition-colors hover:bg-muted/70">
                Get started
              </Link>
            </div>
            <div className="relative flex-1 rounded-[24px] bg-coral-mid p-8 text-white shadow-2xl">
              <div className="absolute -top-3 right-8 rounded-full bg-deep-purple px-3 py-1 text-[10px] font-semibold tracking-wider uppercase">Most popular</div>
              ...
              <ul className="mt-8 space-y-4 text-sm">
                {['Unlimited workspaces', 'Unlimited posts', 'AI images & video reels', 'Priority support'].map((f) => (
                  <li key={f} className="flex items-center gap-2"><Check className="size-4" />{f}</li>
                ))}
              </ul>
              <Link href="/login" className="mt-10 block w-full rounded-full bg-white py-2.5 text-center text-sm font-medium text-coral-mid shadow-xl transition-transform active:scale-95">
                Start free trial
              </Link>
            </div>
          </div>
    ...
    <section className="mx-auto max-w-7xl px-6 pb-24">
      <div className="relative overflow-hidden rounded-[32px] bg-zinc-900 px-8 py-20 text-center">
        <div className="relative z-10 mx-auto max-w-2xl">
          <h2 className="font-heading text-4xl font-medium text-white md:text-5xl">Ready to reclaim your time?</h2>
          <p className="mt-4 text-zinc-400">Join the businesses already saving hours every week with Soshio.</p>
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Link href="/login" className="inline-flex items-center gap-2 rounded-full bg-coral-mid py-3 pr-6 pl-5 text-base font-medium text-white transition-transform hover:-translate-y-0.5">
    ...
    <footer className="border-t border-border py-12">
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-center">
          <div>
            <span className="font-heading text-lg font-semibold tracking-tight text-deep-purple">Soshio</span>
            <p className="mt-2 text-sm text-muted-foreground">Effortless social posting for creators.</p>
          </div>
        ...
        <div className="mt-12 text-xs text-muted-foreground">© {new Date().getFullYear()} Soshio. All rights reserved.</div>
      </div>
    </footer>
  );
}
